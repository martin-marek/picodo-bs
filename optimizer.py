...
def get_optimizer(c: OmegaConf, params, num_microbatch_steps: int, tokens_per_microbatch: int):
    ...
    if c.optimizer == 'adamw':
        ...
        wd_mask = jtu.tree_map_with_path(lambda path, _: path[-2].key=='kernel', params)
    ...
    if c.optimizer == 'muon':
        ...
        optimizer = optimizer_factory(lr_schedule, beta=c.b1, adam_b1=c.b1, adam_b2=c.b2, adam_weight_decay=c.weight_decay, eps=c.eps)
